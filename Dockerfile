...
# Transport is iroh (QUIC, dial-by-key): the hub makes an OUTBOUND connection to
# the iroh relays and is reached by peers through them (or a hole-punched direct
# path), so there is no inbound port to publish — `EXPOSE`/`[http_service]` are
# gone. The hub prints a connection TICKET on start (see the logs); peers
# `clone`/`sync`/`watch --peer` against that ticket.
#
# On startup:
...
#   ASP_RELAY_URL         pin a self-hosted `asp relay` instead of the public n0
#                         relays (optional; default is the public relays).
#   ASP_AUTH_KEY          enrollment secret read by `watch` (clap env) — admits a
#                         not-yet-authorized peer that presents it.
...
CMD ["/bin/sh", "-c", "VAULT_DIR=\"${ASP_DIR:-/mnt/workspace/vault}\" && export ASP_HOME=\"${ASP_HOME:-/mnt/workspace/.asp-home}\" && { [ \"$ASP_RESET\" = \"1\" ] && echo 'ASP_RESET=1: wiping vault history' && rm -rf \"$VAULT_DIR\" || true; } && mkdir -p \"$VAULT_DIR\" \"$ASP_HOME\" && { [ -f \"$VAULT_DIR/.asp/asp.db\" ] || asp init \"$VAULT_DIR\"; } && exec asp watch --dir \"$VAULT_DIR\" --listen"]
